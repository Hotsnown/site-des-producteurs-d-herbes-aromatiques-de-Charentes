...
class IssueEdit extends React.Component {
    ...
    fetch(`/api/issues/${this.props.params.id}`, {
      ...
      if (response.ok) {
        response.json().then(updatedIssue => {
          ...
          this.props.showSuccess('Stock mis à jour');
        });
      } else {
        response.json().then(error => {
          this.props.showError(`Echec: ${error.message}`);
        });
      }
    }).catch(err => {
      this.props.showError(`La communication des données au serveur a échoué : ${err.message}`);
    });
  }
  ...
  loadData() {
    ...
    .then(data => {
      ...
      this.props.showError(`L'extraction des données du serveur a échoué : ${err.message}`);
    });
  }
    ...
    if (Object.keys(this.state.invalidFields).length !== 0 && this.state.showingValidation) {
      validationMessage = (
        <Alert bsStyle="danger" onDismiss={this.dismissValidation}>
          Prière de former une requête valide.
        </Alert>
      );
    }
    return (
      <Panel header="Modifier le stock">
        <Form horizontal onSubmit={this.onSubmit}>
          ...
          <FormGroup>
            <Col componentClass={ControlLabel} sm={3}>Date de l'ajout du stock</Col>
            <Col sm={9}>
              <FormControl.Static>
          ...
          <FormGroup>
            <Col componentClass={ControlLabel} sm={3}>Statut du stock</Col>
            <Col sm={9}>
              <FormControl
          ...
          <FormGroup>
            <Col componentClass={ControlLabel} sm={3}>Producteur</Col>
            <Col sm={9}>
              <FormControl name="owner" value={issue.owner} onChange={this.onChange} />
            </Col>
          </FormGroup>
          <FormGroup>
            <Col componentClass={ControlLabel} sm={3}>Quantité (en grammes)</Col>
            <Col sm={9}>
              <FormControl
          ...
          <FormGroup validationState={this.state.invalidFields.completionDate ? 'error' : null}>
            <Col componentClass={ControlLabel} sm={3}>Date de la requête</Col>
            <Col sm={9}>
              <FormControl
          ...
          <FormGroup>
            <Col componentClass={ControlLabel} sm={3}>Plante</Col>
            <Col sm={9}>
            ...
            <Col smOffset={3} sm={6}>
              <ButtonToolbar>
                ...
                <LinkContainer to="/issues">
                  <Button bsStyle="link">Retour</Button>
                </LinkContainer>
              </ButtonToolbar>
            </Col>
          </FormGroup>
